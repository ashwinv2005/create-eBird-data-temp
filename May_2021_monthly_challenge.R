...
rawpath = "ebd_IN_202105_202105_relMay-2021.txt"
...
############# to check for the basic no Xs, at least 31 lists
...
data5 = data4 %>% filter(count >= 31)


############## to check for lists with breeding codes; removed H and F because they are not really breeding codes

data6 = data %>% filter(!is.na(BREEDING.CODE),!BREEDING.CODE %in% c("H","F")) %>% 
  filter(SAMPLING.EVENT.IDENTIFIER %in% data3$SAMPLING.EVENT.IDENTIFIER)

data7 = data6 %>% group_by(OBSERVER.ID) %>% summarize(bccount = n_distinct(SAMPLING.EVENT.IDENTIFIER)) %>%
  filter(bccount >= 5)

data5 = data5 %>% filter(OBSERVER.ID %in% data7$OBSERVER.ID)

data5 = left_join(data5,data7)
...
write.csv(data5, "may-2021-challenge-results.csv", row.names = F)


##########

a = read.csv("may-2021-challenge-results.csv")
