...
rawpath = "ebd_IN_202012_202012_relDec-2020.txt"
...
############# to check for the basic no Xs, one list every weekday.
...
data3 = data3 %>% filter(!daym %in% c(5,6,12,13,19,20,26,27))

data4 = data3 %>%
  group_by(OBSERVER.ID) %>% summarize(count = n())

data5 = data4 %>% filter(count == (thresh-8))



############## to check for an extra list from 5,6 and 12,13 and 19,20 and 26,27 ##########
...
data7 = data2 %>%
  filter(daym %in% 12:13) %>%
  group_by(OBSERVER.ID) %>% summarize(count = n_distinct(SAMPLING.EVENT.IDENTIFIER)) %>%
  filter(count >= 3)

data8 = data2 %>%
  filter(daym %in% 19:20) %>%
  group_by(OBSERVER.ID) %>% summarize(count = n_distinct(SAMPLING.EVENT.IDENTIFIER)) %>%
  filter(count >= 3)

data9 = data2 %>%
  filter(daym %in% 26:27) %>%
  group_by(OBSERVER.ID) %>% summarize(count = n_distinct(SAMPLING.EVENT.IDENTIFIER)) %>%
  filter(count >= 3)

data5 = data5 %>%
  filter(OBSERVER.ID %in% data6$OBSERVER.ID) %>%
  filter(OBSERVER.ID %in% data7$OBSERVER.ID) %>%
  filter(OBSERVER.ID %in% data8$OBSERVER.ID) %>%
  filter(OBSERVER.ID %in% data9$OBSERVER.ID)
...
write.csv(data5, "december-2020-challenge-results.csv", row.names = F)

a = read.csv("december-2020-challenge-results.csv")
a = a %>% filter(FULL.NAME != "MetalClicks Ajay Ashok")
set.seed(n)
sample(a$FULL.NAME, 1)

#winner: Jayadev Menon
