...
rawpath = "ebd_IN_202011_202011_relNov-2020.txt"
...
############## to check for an extra list from 5 to 12 ##########

data6 = data2 %>%
  filter(daym %in% 5:12) %>%
  group_by(OBSERVER.ID,daym) %>% summarize(count = n_distinct(SAMPLING.EVENT.IDENTIFIER)) %>%
  filter(count >= 2) %>%
  group_by(OBSERVER.ID) %>% summarize(count = n()) %>%
  filter(count == 8)
...
write.csv(data5, "november-2020-challenge-results.csv", row.names = F)

a = read.csv("november-2020-challenge-results.csv")
